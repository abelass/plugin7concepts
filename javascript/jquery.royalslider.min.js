		...
		updateSliderSize: function(force) {
			...
			if(force || newWidth != self.width || newHeight != self.height) {
				...
				for(i = 0; i < self.slides.length; i++) {
					...
					self._slidesContainer.find('.rsImg').css({'margin-left': self._sliderOverflow.width()/2-self._slidesContainer.find('.rsImg').width()/2 -6 + 'px'});
				...
				$('.rsGCaption').css({'bottom':$('.rsThumbs').height()+15+'px', 'border': 'none', 'width': $('.rsSlide').width()+'px', 'margin-left': '5px'});
		...
		_resizeImage:function(slideObject, useClone) {
			...
			if(imgScaleMode === 'fill' || imgScaleMode === 'fit') {		
				...
				if(self.isFullscreen) {
					nWidth = Math.ceil(baseImageWidth * ratio*.85 , 10);
					nHeight = Math.ceil(baseImageHeight * ratio*.85, 10);}
				else{
			...
			if(imgScaleMode !== 'none') {
				...
				cssObj.marginTop = Math.floor((containerHeight - nHeight) / 2)-5 +  bMargin;
			
		
			img.css(cssObj);
				//HACK CAPTION POSITION ON IMG:
	...
	$.extend($.rsProto, {
		...
		enterFullscreen: function(preventNative) {
			...
			setTimeout(function() {	
				...
				//self._slidesContainer.css({'margin-left': self._sliderOverflow.width()/2-$(self.slides[0]).outerWidth()/2 + 'px'});
			}, 300);
		...
		exitFullscreen: function(preventNative) {
			...
			setTimeout(function() {
				...
				$('.rsContainer').css({'margin-left': 'auto'});
			}, 300);
